( function ( mw, $, undefined ) {
	mw.ext.forms.widget.edit.Pasword = function( cfg ) {
		...
		this.strongRegex = new RegExp( '(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[^A-Za-z0-9])(?=.{8,})' );
		this.mediumRegex = new RegExp( '((?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[^A-Za-z0-9])(?=.{6,}))|((?=.*[a-z])(?=.*[A-Z])(?=.*[^A-Za-z0-9])(?=.{8,}))' );
	...
	mw.ext.forms.widget.edit.Pasword.prototype.onChange = function( value ) {
		...
		if ( this.isStrong( value ) ) {
			this.strengthGauge.setLabel( mw.message( 'mwstake-formengine-edit-password-strenght-indicator-stong' ).text() );
			this.strengthGauge.$element.css( { 'background-color': 'green' } );
			this.currentStrength = 3;
		} else if ( this.isMedium( value ) ) {
			this.strengthGauge.setLabel( mw.message( 'mwstake-formengine-edit-password-strenght-indicator-medium' ).text() );
			this.strengthGauge.$element.css( { 'background-color': 'orange' } );
			this.currentStrength = 2;
		} else {
			this.strengthGauge.setLabel( mw.message( 'mwstake-formengine-edit-password-strenght-indicator-weak' ).text() );
	...
	mw.ext.forms.widget.edit.Pasword.prototype.passwordsNotEmptyAndMatch = function() {
		var mainVal = this.main.getValue().trim(),
	...
	mw.ext.forms.widget.edit.Pasword.prototype.setErrors = function( errors ) {
		...
		for ( var i = 0; i < errors.length; i++ ) {
			var widget = new OO.ui.MessageWidget( { type: 'error', inline: true, label: errors[i] } );
