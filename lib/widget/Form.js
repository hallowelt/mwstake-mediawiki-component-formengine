...
console.log( this.definition );
	...
	mw.ext.forms.widget.Form.prototype.processDefinition = function( definition ) {
		...
		if ( this.forInclusion && !this.definition.includable ) {
			this.emit(
				'inclusionError',
				mw.message( 'forms-inclusion-error-form-not-includable', this.definitionName ).text()
			);
			return;
		}
	...
	mw.ext.forms.widget.Form.prototype.convertToJS = function( raw ) {
		for( var key in raw ) {
			if ( Array.isArray( raw[key] ) ) {
				for( var i = 0; i < raw[key].length; i++ ) {
					mw.ext.forms.widget.Form.prototype.convertToJS( raw[key][i] );
				}
				continue;
			}
			var value = raw[key];
			if ( typeof value !== 'string' ) {
				if ( $.type( value ) === 'object' ) {
					mw.ext.forms.widget.Form.prototype.convertToJS( value );
				}
			} else {
				if ( value.startsWith( 'jscb:' ) ) {
					...
					raw[key] = this.funcFromString( value );
				} else {
	...
	mw.ext.forms.widget.Form.prototype.funcFromString = function( cb ) {
		var parts = cb.split( '.' );
	...
	mw.ext.forms.widget.Form.prototype.wireFormButtons = function() {
		...
		if ( this.enableProgressSave ) {
			this.progressSaveButton.on( 'click', this.saveProgress.bind( this ) );
		}
	};
	...
	mw.ext.forms.widget.Form.prototype.canCancel = function() {
		...
		// TODO: Refactor all this, add additional fields for data origin
			...
			this.invokeFormListeners( 'beforeSubmitData', data ).done( function( processedData ) {
				...
				this.runApiCall( apiData, true ).done( function( response ) {
					if ( response.success === 1 ) {
						...
						if ( !silent ) {
							this.successMessage( mw.message( 'forms-form-submit-success' ).text(), true );
						}
						if ( !saveOnly ) {
						...
						if ( response.hasOwnProperty( 'result' ) && response.result.hasOwnProperty( 'id' ) ) {
							...
							this._id = parseInt( response.result.id );
							this.action = 'edit';
						}
				...
				var error = mw.message( 'forms-api-generic-error' ).text();
	...
	mw.ext.forms.widget.Form.prototype.handleSessionLoss = function() {
		OO.ui.alert( mw.message( 'forms-session-loss-error' ).text() );
