( function ( mw, $, undefined ) {
	...
	mw.ext.forms.formElement.TextArea.prototype.getElementConfig = function() {
		var defaults = mw.ext.forms.formElement.TextArea.parent.prototype.getElementConfigInternal.call( this );
		defaults.rows = {
			type: 'number',
			name: 'rows',
			label: mw.message( 'ext-forms-label-number-of-rows' ).text(),
			value: 2,
			widget_data: {
				tab: 'styling'
			}
		...
		defaults.placeholder = {
			type: 'text',
			name: 'placeholder',
			label: mw.message( 'ext-forms-label-placeholder' ).text(),
			widget_data: {
				tab: 'styling'
			}
		};
