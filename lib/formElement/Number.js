( function ( mw, $, undefined ) {
	...
	mw.ext.forms.formElement.Number.prototype.getElementConfig = function() {
		var config = mw.ext.forms.formElement.Number.parent.prototype.getElementConfigInternal.call( this );
		config.min = {
			type: 'number',
			name: 'min',
			value: 1,
			label: mw.message( 'ext-forms-label-min' ).text(),
			widget_data: {
				tab: 'main'
			}
		};
		config.max = {
			...
			name: 'max',
			value: 9999,
			label: mw.message( 'ext-forms-label-max' ).text(),
			widget_data: {
				tab: 'main'
			}
		};
