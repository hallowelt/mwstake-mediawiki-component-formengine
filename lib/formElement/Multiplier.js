( function ( mw, $, undefined ) {
	...
	mw.ext.forms.formElement.Multiplier.prototype.getElementConfig = function() {
		var config = mw.ext.forms.formElement.Multiplier.parent.prototype.getElementConfigInternal.call( this );
		config.addInitially = {
			type: 'checkbox',
			name: 'addInitially',
			value: true,
			label: mw.message( 'ext-forms-label-add-first' ).text()
		};
		config.max = {
			type: 'number',
			name: 'max',
			...
			max: 100,
			value: 5,
			label: mw.message( 'ext-forms-label-max-number-to-add' ).text()
		};
	...
	mw.ext.forms.formElement.Multiplier.prototype.getWidgets = function() {
		return mw.ext.forms.widget.edit.Multiplier;
	};
