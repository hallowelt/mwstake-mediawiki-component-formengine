( function ( mw, $, undefined ) {
	...
	mw.ext.forms.formElement.BookletLayoutPage.prototype.getElementConfigInternal = function() {
		var config = mw.ext.forms.formElement.BookletLayoutPage.parent.prototype.getElementConfigInternal.call( this ),
			newConfig = {
				name: config.name,
				label: {
					type: 'text',
				...
				icon: {
					type: 'text',
					name: 'icon',
					label: mw.message( 'ext-forms-label-icon' ).text()
				}
			};
	...
	mw.ext.forms.formElement.BookletLayoutPage.prototype.getDisplayName = function() {
		return mw.message( 'form-formelement-booklet-layout-page' ).text();
	};
