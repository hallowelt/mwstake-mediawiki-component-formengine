( function ( mw, $, undefined ) {
	mw.ext.forms.formElement.InputFormElement = function() {};

	OO.inheritClass( mw.ext.forms.formElement.InputFormElement, mw.ext.forms.formElement.FormElement );

	mw.ext.forms.formElement.InputFormElement.prototype.getGroup = function() {
		return 'input';
	};

	mw.ext.forms.formElement.InputFormElement.prototype.getElementConfigInternal = function() {
		var config = mw.ext.forms.formElement.InputFormElement.parent.prototype.getElementConfigInternal.call( this );
		config.value = {
			type: 'text',
			name: 'value',
			label: mw.message( 'ext-forms-label-default-value' ).text()
		};

		config.required = {
			type: 'checkbox',
			name: 'required',
			label: mw.message( 'ext-forms-label-required' ).text()
		};

		return config;
	};
} )( mediaWiki, jQuery );