( function ( mw, $) {
	...
	mw.ext.forms.standalone.Form.prototype.render = function() {
		...
		if ( !this.config.hasOwnProperty( 'definition' ) ) {
		...
		if ( this.config.hasOwnProperty( 'definition' )  && this.config.definition.hasOwnProperty( 'listeners' ) ) {
			listeners = this.config.definition.listeners;
		}
